...
\chapter{Rendergleichung}	
	...
	Neben der einfachen Emission- und Absorptionsgleichung, die eine starke Approximation der physikalischen
...
% %	Diese Arbeit erweitert die Beleuchtung der Szene. 
	...
	Wird in die Gleichung \ref{eq:längeLT} der Strahl \ref{eq:Strahl} eingesetzt und die gesamte Gleichung aus multipliziert, dann lässt sich das Renderintegral für die Absorption durch die folgende Gleichung vereinfacht repräsentieren:
...
\section{Aufbau des Programms}
	...
	Die Hauptklasse hat den selben Namen wie das Programm, sie heißt \textit{zikade} und besteht im
	wesentlichen aus zwei Methoden. Die Methode \textit{loadScene} ließt die Textdatei ein, welche
	...
	Variablen vom \textit{real3} entsprechen dreidimensionalen Vektoren, deren einzelne Einträge dem
...
\chapter{Auswertung und Fazit}
	...
	In diesem Kapitel werden zunächst qualitative unterscheide in Abhängigkeit der gewählten Parameter erläutert
	und im Anschluss wird die beschleunigte Variante des Algorithmus evaluiert.
	...
	\begin{figure}[h]
		...
		\caption{Abbildungen einer einfachen Szene gezeichnet mit dem Phong-Beleuchtungsmodell,
				 einmal ohne und einmal mit globalen Schatteneffekten. Zusätzlich, wird die
				 selbe Szene, gezeichnet mit dem einfachen Emissions- und Absorptionsmodell dargestellt.}
	...
	schattiert wird.
	Anders verhält es sich im dritten Bild (c) der Abbildungen \ref{fig:vergleich1}, dort wird die gleiche Szene durch das einfachen Emissions- und Absorptionsmodell dargestellt. 
	...
	\begin{figure}[ph]
		...
		\caption{Die Abbildung zeigt die selbe Szene wie die Abbildung \ref{fig:vergleich1},
				 gezeichnet mit dem erweiterten Emissions- und Absorptionsmodell}
		\label{fig:vergleich2}
	\end{figure}
	
	Die Bilder (a) und (b) in der Abbildung \ref{fig:vergleich2} zeigt die gleiche Szene wie zuvor,
	...
	Bevor dir Geschwindigkeit des Algorithmus betrachtet wird, werden numerische Grenzfälle betrachtet.
	...
	gewählt, als in den bereits gezeigten Abbildungen. Im Fall des Bildes (a) aus der Abbildung  \ref{fig:numerischeGrenzfälle1} wird gar kein Stützpunkt verwendet, was zur folge hat,
	...
	und im Bild (b) auf $1.0$, so dass die Kugel überhaupt kein Licht mehr durchlässt.
...
\section{Fazit}
	...
	Ein Problem der Implementierung besteht darin, dass es zu Artefakten kommt, wenn sich Kugeln überlappen.
	...
	Auf die Implementierung von Brechungseffekten wurde aus zwei folgenden Gründen bewusst verzichtet.
