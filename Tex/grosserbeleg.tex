...
\section{Erweitertes Dichte Modell}
	...
	Die Lösung der Gleichung wird mit Hilfe von Riemann Summe numerisch approximiert. Bevor die Lösung präsentiert
	...
	Das Intervall von $0$ bis $D$ wird in $n$ kleinere Teile zerlegt, jedes dieser Teilintervalle besitzt die selbe Länge $\Delta x = \frac{D}{n}$. 
	...
	Es wird eine Anzahl von $n$ Stützpunkten zur Regulierung der Güte der Approximation definiert. Generell gilt,
	je größer der Wert von $n$ ist um so besser wird das Integral approximiert. 
	Der Wert für die Skalierung bestimmt sich als $\Delta t = \frac{t_f - t_n}{n}$.
	
	Im Folgenden entspricht die Menge $L = \{L_1, L_2, \cdots L_m\}$ der Menge der Lichtquellen, jede von ihnen wird über
	den dazugehörigen Index identifiziert. 
	Die Intensitäten der Lichtquellen werden mit dem Formelzeichen $I_{L_i}$ symbolisiert und $i$ entspricht dabei dem Index der dazugehörigen Lichtquelle.
	Damit kann der Quellterm erweitert werden und die Formel für die Beleuchtungsberechnung entspricht:
...
\chapter{Implementation}
	...
	In diesem Kapitel werden die Grundlagen und die beiden Beleuchtungsmodelle zusammengeführt und es wird
	erläutert, wie die Bilder im Detail erzeugt werden.
	
	Die Auflösung eines Bildes wird mit der Breite $X$ und der Höhe $Y$ beschrieben.
	Für jeden Bildpunkt $(x, y) \in X \times Y$ wird ein Strahl erzeugt und in einer Liste gespeichert. 
	Der Ursprung jedes primären Strahlen entspricht der Kameraposition und die Richtungsvektoren lassen
	sich in Abhängigkeit der Bildposition $(x, y)$ und der fokalen Länge $f$ anhand der Vorschrift \ref{eq:direction} bestimmen.
	Die Kamera kann wie in den Grundlagen beschrieben, durch eine Rotation der Richtungsvektoren, 
	jedes primären Strahlen gedreht werden.
	Das Resultat dieses Schrittes ist eine Liste der primären Strahlen.
	
	Die zu Zeichnende Szene besteht aus einer Menge von Kugeln, 
	welche entsprechen der Grundlagen sich durch eine Position und einen Radius parametrisiert werden.
	Zusätzlich wird für jede Kugel ein Wert für die Maximale Verdeckung angegeben. 
	Dieser Wert wird durch das Symbol $\Theta_{max}$ repräsentiert.
