\documentclass[hyperref,german,beleg,final,lof, lot]{cgvpub}
...
\section{Motivation}
	...
	Anders verhält es sich bei Simulationen, in diesen werden Partikel zur Beschreibung physikalischer
	Vorgänge verwendet und bedingen sich sehr wohl. 
	Computergestütze Simulationen zählen zur größten Datenquelle für Partikeldaten, 
	daneben werden diese bei Messungen, insbesondere in der Biologie und der Atomaren Physik erhoben.
	Eine direkte und unverfälschte Darstellung dieser Daten ist dabei ein entscheidendes Ziel.
	
	Das Zweck von Visualisierungen besteht im allgemeinen, in der Unterstützung von der menschlichen Wahrnehmung, 
	bei der Auswertung und Analyse der dargestellten Datenmenge. 
...
\chapter{Darstellungsmöglichkeiten}

	Es gibt eine Reihe von Verfahren um mit Hilfe eines Computers geometrische Objekte auf eine Bildfläche abzubilden.
	Die wohl am häufigsten in der Computergrafik eingesetzte Technik ist der Rasterizierung.
	Um eine beliebige beschriebene geometrische Form mit Hilfe dieser Technik abzubilden ist eine Zerlegung
	in ein polygonales Netz erforderlich. 
	Das bedeutet, das die eigentliche Geometrie in Primitiven, wie Punkte, Linien oder Dreiecke zerlegt wird.
	Diese werden dann auf die Bildschirmfläche abgebildet.
	Im Anschluss werden die sogenannten Fragmente, die auf die Pixel abgebildeten Polygone, nach einem beliebigen
	Verfahren zur eigentlichen Pixelfarbe verrechnet.
	Ein großer Vorteil dieser Methode ist die Geschwindigkeit, mit der die Bilder aus der Geometrie erzeugt werden
	können.
	

\chapter{Beleuchtungsmodelle}
...
\section{Erweitertes Beleuchtungs-Modell}
	...
	Die einfache Variante entstammt der Arbeit \ref{}, in welcher zusätzlich eine Implementierung
