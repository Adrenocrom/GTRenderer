...
\chapter{Grundlagen}
	...
	einen Punkt und $\vec{v}$ einen Vektor und der Term $\langle\vec{a}, \vec{b}\rangle$ bezeichnet ein Skalarprodukt.
...
\section{Die Perspektivische Kamera}
	...
	lässt sich Bildebene Vor dem Loch positionieren, damit das entstandene Bild nicht gespiegelt aufgenommen wird. Die Position $\underline{p}$ der Kamera ist mit der Position des Lochs der \textit{Camera obscura} identisch.
...
\chapter{Ein analytischer Ansatz}
	...
	Gibt es eine analytische Lösung, so würden sich die Bilder effizient und korrekt berechnen lassen.
	Lässt sich Gleichung nicht analytisch Lösen, werden die Integrale so weit wie möglich analytisch gelöst
	und die restlichen Teile der Gleichung müssen Nummerisch approximiert werden.
...
\subsection{Emission}
	...
	\begin{figure}[h]
		...
		% %\includegraphics{images/standartAbsorption2.pdf}
		\caption{Veranschaulichung des Strahlengangs durch die Kugel, die Nullstellen sind mit $t_n$ und $t_f$ bezeichnet. Der Gelbe Kreis auf der rechten Seite stellt eine Lichtquelle dar.}
		\label{fig:simpleBackgroundTransperency}
	\end{figure}
