...
\subsection{Absorption und Emission}
	...
	Dabei wird das Licht als Strahl von der Lichtquelle durch das Medium geschickt. Die Variable $s$ entspricht dem Laufparameter des Strahls,
...
\section{Streuung und Schatten}
	...
	trifft wird gestreut, es entstehen Schatten. Nelson Max geht dabei auf die Grundlagen der Streuung ein, in dem der Quellterm $g(s)$ um einen
	Winkelparameter erweitert wird. Der Quellterm wird im Folgenden mit $g(X, \omega) = E(X) + S(X, \omega)$. $E(X)$ bezeichnet die Emission an einen
	bestimmten Punkt $X$ und $S(X, \omega)$ die Richtung in welche das Licht gestreut wird. 
