...
\chapter{Einleitung}
	...
	Nach dem Details zur eigentlichen Implementierung Vorgestellt worden, wird das Verfahren Qualitativ und
	Quantitativ evaluiert und es wird auf mögliche Erweiterungen im Schlusswort eingegangen.
...
\chapter{Volumenintegral}	
	...
	Geometrische Objekte werden in einen Vorverarbeitungsschritt, meist in polygonale Netze zerlegt.
...
\section{Rendergleichung}
	...
	Grundlage dieser Gleichung, sind physikalischen Modelle, die die Wechselwirkungen von Licht mit Materie beschreiben.
	...
	Die Ausbreitung des Lichtes wir mit Lichtstrahlen beschrieben. 
	...
	So können sich zwei Lichtstrahlen schneiden, ohne das die Kollision ihre Energie oder ihren Weg beeinflusst. 
	...
	Neben der einfachen Emission- und Absorptionsgleichung, die eine starke eine Approximation der physikalischen
	Grundlagen darstellt, geht Nelson Max auf diverse Erweiterungen zur Berechnung von Streuungseffekten und Schatten ein.
	Der Begriff Albedo bezeichnet ein Maß für das Rückstrahlvermögen, das bedeutet das die Oberflächen das eingestrahlte Licht nur diffuse reflektieren.
	...
	Auf eine Phasenfunktion, beziehungsweise eine bidirektionale Reflektanzverteilungsfunktion wir ebenfalls vernachlässigt.
	
\subsection{Emission und Absorption}

	Ausgangspunkt ist das Emission-Absorption-Modell von Nelson Max. In diesem werden die Wechselwirkungen des Lichtes auf die Absorption und die Emission beschränkt. Die Streuung des Lichtes wird wie oben beschrieben vernachlässigt.
...
\subsection{Einfaches Beleuchtungs-Modell}
...
\subsection{Erweitertes Beleuchtungs-Modell}
	...
	Dieser Arbeit erweitert die Beleuchtung der Szene. 
	...
	Daraus resultiert, dass die Wahrscheinlichkeit, das ein Sekundärstrahl eine solche Lichtquelle treffen könnte
	...
	Diese lässt sich durch die folgende Vorschrift berechnet:
	...
	Wird in die Gleichung \ref{eq:längeLT} der Strahl \ref{eq:Strahl} eingesetzt und die gesamte Gleichung aus multipliziert, dann lässt sich Renderintegral für die Absorption durch die folgende Gleichung vereinfacht repräsentieren:
	...
	approximiert werden. Dazu dieser wird das Integral in Riemann Summen zerlegt.
...
\chapter{Implementierung}
	...
	Zusätzlich wird für jede Kugel ein Material definiert, welches sich aus einer Farbe und ein Wert für die maximale Lichtundurchlässigkeit zusammensetzt. 
	...
	sondern berechnet sich aus der angegeben maximalen Lichtundurchlässigkeit $\Theta_{max} \in [0, 1]$ in Abhängigkeit
	des Radius $r$.
	...
	auflösen.
	Zusätzlich wird die größte mögliche Distanz $2 \cdot r$ für die Länge $t_f - t_n$ gesetzt und
	...
	Das Programm unterstützt eine Vielzahl von Einstellungsmöglichkeiten, diese werden in einer externen
	Textdatei gesetzt. Deren Aufbau und mögliche Parameter werden im Folgenden Abschnitt erläutert.

\section{Szenenbeschreibung}

	Wie oben beschrieben, lässt sich die Szene und die Programmparameter durch eine Textdatei definieren.
	...
	In SIFF-Dateien können die Kugeln durch ihrer Position, ihren Radius und ihrer Farbe ebenfalls Zeilenweise beschrieben werden.
	...
	Weitere globale Parameter, wie $\lambda$ und die Anzahl von Stützpunkten lassen sich ebenfalls setzten.
	...
	Was es mit diesen Parameter auf sich hat, wird später beschrieben.
	...
	Die Punktlichtquellen besitzen zusätzlich eine Position
	und Richtungslichtquellen besitzt anstatt der Position einen normierten Richtungsvektor.
	
	Die Beleuchtungsbrechung lässt sich neben dem erläuterten Emission- und Absorptionsmodell auch mit dem
...
\section{Details zum Algorithmus}
	...
	kommt.
	Grundlegend gilt, das die Auflösung, des zu erzeugenden Bildes $W \times H$ entspricht.
	Unabhängig vom Beleuchtungsmodell, wird für jeden Bildpunkt $(x, y) \in W \times H$ wird ein primärer Strahl erzeugt, dessen Ursprung der Kameraposition entspricht.
	...
	Für jeden Primärstrahl, Sichtstrahl wird der Schnitt mit allen in der Szene befindlichen Kugeln berechnet
	...
	geschossen werden und wenn ein Sekundärstrahl dabei eine andere Kugel schneidet, dann wird die Farbe
	der Lichtquelle mit dem Nullvektor ersetzt.
	
	Die Bildsynthese für das Emission- und Absorptionsmodell ist etwas aufwendiger.
	Für die Sichtstrahlen wird die Beleuchtung durch das erweiterte Beleuchtungsmodell berechnet. 
	Dazu muss ebenfalls für jeden Strahl ein Schnittest mit allen Kugeln durchgeführt werden.
	...
	Werden globale Schatten verwendet werden, dann wird dieses Licht entlang des Strahls wiederum durch die vom
	Sekundärstrahl geschnittenen Partikel abgeschwächt. 
	Diese Abschwächung wird mit vereinfachten Beleuchtungsmodell berechnet.
	Für jeden Sekundärstrahl ist ein erneuter Schnittest mit allen Kugeln nötigt, 
	sowie eine Sortierung der Ergebnismenge entlang des Sekundärstrahls. 
	
	Die Geschwindigkeit des Algorithmus hängt von der Auflösung des Bildes und der damit verbunden Anzahl von Primärstrahlen ab.
	Entscheidend für Berechnungsdauer ist die Wahl der Stützpunktanzahl, 
	da für jeden ein Sekundärstrahl Szene geschossen werden muss und für jeden ein Schnitttest und eine anschließende
	Sortierung erforderlich ist.
	
	An dieser Stelle wird bereits angesprochene Approximationsgrad als ein neuer Parameter $N$ eingeführt. 
	...
	Stützpunktanzahl reduziert wird, bis schließlich das Vereinfachte Modell zum Einsatz kommt.
	In diesen Programm wurde die Anzahl nicht adaptiv gewählt, sondern nur die einfache Approximation implementiert.
	Der Einfluss und die Güte dieser Approximation wird im folgenden Kapitel anhand von Beispielen evaluiert.
...
\chapter{Auswertung und Fazit}
	...
	Es wurde eine Vereinfachung zur Beschleunigung der Bildsynthese vorgestellt.
	Diese wird im Folgenden mit Ergebnissen verglichen, die ohne Vereinfachung erzeugt worden sind.  
...
\section{SSIM}
	...
	Das Problem  besteht darin, das diese Verfahren Kontext unabhängig sind.
	...
	Der Fehler steigt drastisch an und die beiden Bilder als vollkommen unterschiedlich klassifizieren, 
	...
	Das letztliche Maß lässt durch die folgende Gleichung errechnen:
...
\section{quantitative Evaluation}
	...
	Diese wurde mit Hilfe des \textit{structural similarity}-Index, kurz SSIM ermittelt. 
	Dieser wurden nur für Bilderpaare mit 100 Stützpunkten bestimmt. 
	Bei niedrigeren Anzahlen von Stützpunkten würde der Fehler, der Approximation das Ergebnis zu sehr verfälschen.
	Dabei wird die Übereinstimmung der Ergebnisse, im Verhältnis zum Ground-Truth Bildes für jeden Farbkanal einzeln
	in Prozent angeben. Die nachfolgenden Tabellen wurden um den Mittelwert der Übereinstimmung aller Farbkanäle ergänzt:
	...
	dass die Ergebnisse der beschleunigten Variante des Algorithmus, 
	ab einen gewissen $N$ sich kaum noch von den Ground-Truth Bildern unterscheiden.
